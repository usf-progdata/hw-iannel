...
```{r}
select(gapminder, continent, everything()) #don't need quotes
```
...
```{r}
arrange(gapminder, desc(year)) #desc is a function 
```
...
```{r}
gapminder %>% 
  select(c(year, lifeExp, country)) %>% #works with or without the concatenate function; don't need it using select() 
...
```{r}
gapminder %>%
  filter(country %in% c("Brazil", "China") & pop > 100000000) #%in% is like ==, except it checks against all values in the vector; equal sign will alternate back and forth
```
...
psych::bfi %>%
  ...
  mutate(across(A1:O5, ~ (.x - 1) / 5 * 100)) #do a transformation .x is place holder for A1,2,3,4,5
```
...
gapminder %>% 
  summarize(mu = mean(lifeExp),
            sigma = sd(lifeExp)) #gives mean and standard deviation for enire data set
```
...
psych::bfi %>%
  as_tibble(rownames = ".id") %>% 
  select(.id, A1:A5) %>% #dropping all of the columns except for these 6 
  rowwise() %>% 
  mutate(A_mean = mean(c(A1, A2, A3, A4, A5), na.rm = TRUE)) %>% 
  ungroup() #ungroup afterward to go back to working with whole dataset
```
